...
public class WorkerInfoWindowController implements PersonController, Initializable {
    ...
    public ComboBox cbActionDay;
    public ComboBox cbActionMonth;
    public ComboBox cbActionYear;
    private int editItemIndex;
    private Vector<TextField> textFields = new Vector<>();
    private Vector<ComboBox> comboBoxes = new Vector<>();

    public WorkerInfoWindowController(int index) {
        editItemIndex = index;
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        comboBoxes.addAll(Arrays.asList(cbBirthDay, cbBirthMonth, cbBirthYear, cbActionDay, cbActionMonth, cbActionYear));

        int initialBirthYear = 1970, initialActionYear = 1990, lastBirthYear = 2010, lastActionYear = 2020;

        for (int i = 1; i < 32; i++) {
            cbActionDay.getItems().add(i);
            cbBirthDay.getItems().add(i);
        }
        cbActionDay.getSelectionModel().selectFirst();
        cbBirthDay.getSelectionModel().selectFirst();

        for (int i = 1; i < 13; i++) {
            cbActionMonth.getItems().add(i);
            cbBirthMonth.getItems().add(i);
        }
        cbActionMonth.getSelectionModel().selectFirst();
        ...
        for (int i = initialActionYear; i < lastActionYear; i++) {
            cbActionYear.getItems().add(i);
        }
        cbActionYear.getSelectionModel().selectFirst();
        ...
        if (isEdit()) {
            ...
            try {
                if (rs.next()) {
                    ...
                    cbActionDay.getSelectionModel().select(rs.getInt("buy_sell_day")-1);
                    cbActionMonth.getSelectionModel().select(rs.getInt("buy_sell_month")-1);
                    cbActionYear.getSelectionModel().select(rs.getInt("buy_sell_year") - initialActionYear);
                }
    ...
    public void confirm(ActionEvent actionEvent) {
        ...
        String query = null;
        if (isEdit()) {
            query = "UPDATE `Worker` SET " + "`last_name`='" + tfLastName.getText() + "',`first_name`='" +
                    ...
                    "`appointment_day`='" + cbActionDay.getSelectionModel().getSelectedItem().toString() + "'," +
                    "`appointment_month`='" + cbActionMonth.getSelectionModel().getSelectedItem().toString() + "'," +
                    "`appointment_year`='" + cbActionYear.getSelectionModel().getSelectedItem().toString() +
                    "' WHERE id='" + editItemIndex + "'";
        } else {
            query = "INSERT INTO `Worker`(`last_name`, `first_name`, `patronymic`, `birth_day`, `birth_month`," +
                    "`birth_year`, `passport_series`, `passport_num`, `appointment_day`, `appointment_month`, `appointment_year`)" +
                    ...
                    "','" + cbActionDay.getSelectionModel().getSelectedItem().toString() + "','" + cbActionMonth.getSelectionModel().getSelectedItem().toString() +
                    "','" + cbActionYear.getSelectionModel().getSelectedItem().toString() + "')";
        }
